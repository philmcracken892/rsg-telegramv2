...
AddEventHandler('rsg-telegram:server:SendMessage', function(senderID, sender, sendername, tgtid, subject, message)
    ...
    -- tgtid is now the target player's server ID
...
AddEventHandler('rsg-telegram:server:GetMessages', function(tid)
    ...
    TriggerClientEvent('rsg-telegram:client:MessageData', src, telegram, tid)
end)
...
AddEventHandler('rsg-telegram:server:DeleteMessage', function(tid, silent)
    ...
    if result[1] == nil then
        if not silent then
            TriggerClientEvent('ox_lib:notify', src, {title = "Error", description = "Message not found", type = 'error', duration = 5000 })
        end
        return
    end
    ...
    if not silent then
        TriggerClientEvent('ox_lib:notify', src, {title = "Success", description = "Message deleted", type = 'success', duration = 3000 })
        Wait(500)
...
RSGCore.Functions.CreateCallback('rsg-lawman:server:GetPlayers', function(source, cb)
    local players = {}
